...
import personal.wuqing.rogue.riscv.RVBlock
        ...
        body += function.body.map { block ->
            ...
            when (val it = block.terminate) {
                is IRStatement.Terminate.Ret -> {
                    ...
                    ret.instructions += RVInstruction.JR(ra)
                }
                is IRStatement.Terminate.Branch -> {
                    ret.instructions += boolDef[it.cond]?.let { bool ->
                        RVInstruction.Branch(
                            operator(bool.operator), asRegister(bool.op1), asRegister(bool.op2), it.then.name
                        )
                    } ?: RVInstruction.Branch(
                        RVCmpOp.NE, asRegister(it.cond), RVRegister.ZERO, it.then.name
                    )
                    ret.instructions += RVInstruction.J(it.els.name)
                }
                is IRStatement.Terminate.Jump ->
                    ret.instructions += RVInstruction.J(it.dest.name)
            }
            ret
        }
    }
